...
export const gen_stu = graph_with_symbols.map(G.graph_to_units)

export const gen_units = gen_stu.map(stu => stu.source)

qc('roundtrip string<->units', gen_units, (units, p) =>
  p.equals(units, G.parse_strict(p.tap(G.units_to_string(units))))
)

const gen_init_units = QC.nestring(QC.char(' ab')).map(s => G.graph_to_units(G.init('a' + s)))

qc('init is minimal', gen_init_units, stu => {
  const m = G.minimize(stu)
  const blank = (u: G.Unit) => u.links.length == 0 && u.ids.length == 0
...
qc('init rearrange minimal bound', gen_rearranged_init_units, (r, p) => {
  const m = p.tap(G.minimize(r.stu))
  const not_blank = (u: G.Unit) => u.links.length != 0 || u.ids.length != 0
