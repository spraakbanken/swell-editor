...
describe('Dropdown', () => {
  ...
  it('calls onChange', () => {
    ...
    dom.find('input').simulate('keydown', {key: 'Enter', target: {value: ''}})
    chai.assert.deepEqual(called, ['a', true])
    chai.assert.equal(times, 1)
    dom.find('input').simulate('keydown', {key: ' ', target: {value: ''}})
    chai.assert.deepEqual(called, ['a', true])
    chai.assert.equal(times, 2)
    dom.find('input').simulate('keydown', {key: 'Backspace', target: {value: ''}})
  ...
  it('skips directly to search hits', () => {
    ...
    chai.assert.deepEqual(called, [])
    chai.assert.equal(times, 0)
    dom.find('input').simulate('keydown', {key: 'y', target: {value: ''}})
    dom.find('input').simulate('keydown', {key: 'Enter', target: {value: ''}})
    chai.assert.deepEqual(called, ['xy', true])
    chai.assert.equal(times, 1)
    dom.find('input').simulate('keydown', {key: 'z', target: {value: 'y'}})
    dom.find('input').simulate('keydown', {key: 'Enter', target: {value: ''}})
    chai.assert.deepEqual(called, ['xyz', true])
    chai.assert.equal(times, 2)
    dom.find('input').simulate('keydown', {key: 'w', target: {value: 'yz'}})
    dom.find('input').simulate('keydown', {key: 'Enter', target: {value: ''}})
    chai.assert.deepEqual(called, ['xyz', true])
    chai.assert.equal(times, 3)
  })
