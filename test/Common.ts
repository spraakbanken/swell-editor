...
export const aof = <A>(l: number, u: number, g: Gen<A>) => Gen.between(l,u).then(i => g.replicate(i))
...
export const graph: Gen<Graph> = Gen.size().then(i => Gen.between(1, 2 + Math.round(i / 10))).replicate(2).then(([ssize, tsize]) =>
  ...
  Gen.record({
    ...
    proto_edges: aof(0,2, sof(1,3,Gen.upper)).replicate(esize).map(xs => xs.map(labels => ({ids: [] as string[], labels}))),
